---
title: "Data Analysis of Anatomical Comparison Dataset"
_output: html_notebook
---

```{r setup}
    knitr::opts_knit$set(root.dir ="C:/Users/j789s948/Documents/R_Studio_files/Anatomical_Comparison_GeoMx") 
    
getwd()
```

# 0. Summary of Notebook

This summary incorporates all plates for the anatomical comparison analysis up to plates 9-10, skipping the plates 7 and 8 which had sequencing issues.

QC is done up to the Normalization and output files are prepared to send to Leonidas and team.
...
Eventually, an interactive analysis tool should be developed with R-Shiny to permit the majority of this analysis to be performed by non-R users.
...
-   Annotation file - this will contain information about the tissue, segment area and nuclei count, and any other info you choose to provide.
    ...
    -   For new datasets, use lab worksheet from the readout package.
...
# blank_annotations <- readxl::read_xlsx("C:/Users/j789s948/Documents/GeoMx Data Analysis/input_files/annotations/Blank_Annotations.xlsx")
# 
# main_annotations <- readxl::read_xlsx("C:/Users/j789s948/Documents/GeoMx Data Analysis/input_files/annotations/Main_Annotations_File_10-27-23.xlsx")
# 
# cols_to_merge = c("Sample_ID", "Ovulation", "CM_CoCulture_Control", "Tissue", "Region", "Days", "Treatment", "Patient", "Experiment")
# 
# merged_doc <- base::merge(blank_annotations, main_annotations[ , cols_to_merge], by = "Sample_ID", all.X = TRUE, all.Y= TRUE, sort = FALSE)
...
# writexl::write_xlsx(merged_doc, "C:/Users/j789s948/Downloads/Ordered_Annotations_10-27-23.xlsx")
...
datadir <- file.path("~/R_Studio_files/Anatomical_Comparison_GeoMx/data_input")
...
#Note that we add "Anatomical to ensure we open the correct annotation file
#### IMPORTANT #### make sure to change to appropriate file
SampleAnnotationFile <- dir(file.path(datadir, "annotation"), pattern = "10-27-23.xlsx$",
      full.names = TRUE, recursive = TRUE)
...
Now we need to make a smaller dataset including only the anatomical comparison experiments and excluding plates 7 and 8 (which had sequencing issues).

```{r}

# select only those where Experiment == "anatomical"



AnatCompData <- demoData[, pData(demoData)$Experiment %in% c("anatomical", NA)]

# Also would be a good idea to delete unneeded column names in pData
# Treatment, Days, CM_CoCulture_Control, Ovulation, ShortName
...
pheno_data <- pheno_data[, !colnames(pheno_data) %in% c("Treatment", "Days", "CM_CoCulture_Control", "Ovulation", "ShortName") ]
...
-   Nuclei: \>100 nuclei per segment is generally recommended; however, this cutoff is highly study/tissue dependent and may need to be reduced; what is most important is [consistency in the nuclei distribution for segments within the study.]{.underline}
    ...
    My note: Do they means consistency in count or consistency in average nuclei per area?
...
```{r}
save(AnatCompData_afterQC, file = "AnatCompData_afterQC.Robject")
...
save(AnatCompData_AfterProbeQC, file = "AnatCompData_AfterProbeQC.Robj")
...
save(AnatCompData_collapsedtoTarg, file = "AnatCompData_collapsedtoTarg.Robj")
...
ggplot(test_gr_0.1, aes(x, id = id, split = y, value = n)) +
    ...
    scale_y_continuous(expand = expansion(0)) + 
    scale_x_discrete(expand = expansion(0)) +
    labs(title = "Segment Distribution After QC", x = "", y = "") +
    annotate(geom = "segment", x = 3.3, xend = 3.3,
             y = 0, yend = 100, lwd = 2) +
    annotate(geom = "text", x = 3.2, y = 50, angle = 90, size = 5,
...
ggplot(test_gr_0.05, aes(x, id = id, split = y, value = n)) +
    ...
    scale_y_continuous(expand = expansion(0)) + 
    scale_x_discrete(expand = expansion(0)) +
    labs(title = "0.05 segment filtering", x = "", y = "") +
    annotate(geom = "segment", x = 3.3, xend = 3.3,
             y = 0, yend = 100, lwd = 2) +
    annotate(geom = "text", x = 3.2, y = 50, angle = 90, size = 5,
...
save(Anat_0.1seg_0.1gene, file = "Anat_0.1seg_0.1gene.Robj")

```

Now we need to export the prenormalized data for Leonidas and Co.
...
print_TargetCountMatrix(matrix = Exprs_data_0.05_filter, file_path= "C:/Users/j789s948/Downloads/Exprs_data_0.05_filter.xlsx")


print_TargetCountMatrix(matrix = Exprs_data_0.1_filter, file_path= "C:/Users/j789s948/Downloads/Exprs_data_0.1_filter.xlsx")
```
...
print_anat_comp_annotations(Anat_0.05seg_0.1gene, "C:/Users/j789s948/Downloads/Annotation_0.05_filter.xlsx")


print_anat_comp_annotations(Anat_0.1seg_0.1gene, "C:/Users/j789s948/Downloads/Annotation_0.1_filter.xlsx")
```
...
save(quantile_AnatCompData, file = "C:/Users/j789s948/Documents/R_Studio_files/Anatomical_Comparison_GeoMx/data_output/Quantile_Norm_Anatomical_Comparison_11-21-23.Rdata")


save(quantile_AnatCompData_allgenes, file = "C:/Users/j789s948/Documents/R_Studio_files/Anatomical_Comparison_GeoMx/data_output/Quantile_Norm_Anatomical_Comparison_allgenes_11-26-23.Rdata")
...
# Stat_data <- 
#     data.frame(row.names = colnames(exprs(target_AnatCompData_0.1_genefilter)),
#                Segment = colnames(exprs(target_AnatCompData_0.1_genefilter)),
#                Annotation = pData(target_AnatCompData_0.1_genefilter)[, ann_of_interest],
#                Q3 = unlist(apply(exprs(target_AnatCompData_0.1_genefilter), 2,
#                                  quantile, 0.75, na.rm = TRUE)),
#                Quantile = unlist(apply(assayDataElement(quantile_AnatCompData, "q_norm"), 2,
#                                  quantile, 0.75, na.rm = TRUE)),
#                NegProbe = exprs(target_AnatCompData_0.1_genefilter)[neg_probes, ]
#                
#                )
# Stat_data_m <- melt(Stat_data, measure.vars = c("Q3", "Quantile", "NegProbe"),
...
# target_AnatCompData <- normalize(target_AnatCompData ,
#                              norm_method = "quant", 
#                              desiredQuantile = .75,
#                              toElt = "q_norm")
# 
# # Background normalization for WTA/CTA without custom spike-in
# target_AnatCompData <- normalize(target_AnatCompData2 ,
#                              norm_method = "neg", 
#                              fromElt = "exprs",
#                              toElt = "neg_norm")
...
```{r}
# save(target_AnatCompData, file = "C:/Users/j789s948/Documents/R_Studio_files/Anatomical_Comparison_GeoMx/data_output/Q3_Norm_Anatomical_Comparison_10-2-23.Rdata")
...
# boxplot(exprs(target_AnatCompData)[,1:68],
...
# boxplot(assayDataElement(target_AnatCompData2[,1:68], elt = "q_norm"),
...
# boxplot(assayDataElement(target_AnatCompData2[,1:68], elt = "neg_norm"),
...
# run UMAP
# umap_out <-
#     umap(t(log2(assayDataElement(target_AnatCompData , elt = "q_norm"))),  
#          config = custom_umap)
# pData(target_AnatCompData)[, c("UMAP1", "UMAP2")] <- umap_out$layout[, c(1,2)]
# ggplot(pData(target_AnatCompData),
...
# tsne_out <-
#     Rtsne(t(log2(assayDataElement(target_AnatCompData , elt = "q_norm"))),
#           perplexity = ncol(target_AnatCompData)*.15)
# pData(target_AnatCompData)[, c("tSNE1", "tSNE2")] <- tsne_out$Y[, c(1,2)]
# ggplot(pData(target_AnatCompData),
#        aes(x = tSNE1, y = tSNE2)) +
#     geom_point(aes( color = region, shape = segment), size = 3) +
#     theme_bw()+
#     geom_mark_ellipse(aes(x = tSNE1, y = tSNE2, fill = patient_number), inherit.aes = FALSE)
...
# assayDataElement(object = target_AnatCompData, elt = "log_q") <-
#     assayDataApply(target_AnatCompData, 2, FUN = log, base = 2, elt = "q_norm")
# 
# # create CV function
# calc_CV <- function(x) {sd(x) / mean(x)}
# CV_dat <- assayDataApply(target_AnatCompData,
...
# pheatmap(assayDataElement(target_AnatCompData[GOI, ], elt = "log_q"),
#          scale = "row", 
...
#          annotation_col = 
#              pData(target_AnatCompData)[, c("patient_number", "segment", "region")])
...
![For more details on the LMM, please refer to the lme4 package and the lmerTest package.](images/Linear%20Mixed%20Model%20Example.PNG)

## 7.1 Within Slide Analysis: Ciliated vs Secretory
...
## Exporting specific probe info

```{r}

TXNIP <- AnatCompData_AfterProbeQC[fData(AnatCompData_AfterProbeQC)[["TargetName"]]=="TXNIP"]

```

# Plot Probe Expression

```{r}

Probe_Anat_Boxplot_Points <- function(probe = "RTS0045226", point_size = 3, dataset = ST6GAL1){
  
  # %in% checks to see if a name is in a list. 
  # I define not in (%ni%) to do the opposite (check if NOT in the list)
  `%ni%` <- Negate(`%in%`)
  gene_not_found <- c()
  if(probe %ni% rownames(assayDataElement(dataset, elt = "exprs"))){
    return(paste0(probe, " not in list"))
  }

    plot <- ggplot(
      pData(dataset), 
      aes(x = Region,
          y = as.vector(assayDataElement(dataset[probe, ], elt = "exprs")),
          fill = Region
          )
      )+
      geom_boxplot(outlier.shape = NA) +
      labs(title = TeX(paste0("\\textit{", probe, "} Expression")), 
           y = TeX(paste0("\\textit{", probe, "} Expression")),
           x = "Anatomical Region")+
      facet_wrap(~segment)+
...
txnip_list <- rownames(fData(TXNIP))

lapply(X = txnip_list, FUN = Probe_Anat_Boxplot_Points)
...
ST6GAL1 <- AnatCompData_AfterProbeQC[fData(AnatCompData_AfterProbeQC)[["TargetName"]]=="ST6GAL1"]
...
ST6GAL1_list <- rownames(fData(ST6GAL1))

lapply(X = ST6GAL1_list, data = ST6GAL1, FUN = Probe_Anat_Boxplot_Points)
